...
import { initializeFileLogging, traceError, traceInfo } from './logging';
...
import { LanguageManager } from './erdos/runtime';
import { PythonConsoleCompletionProvider } from './erdos/consoleCompletionProvider';
...
async function activateUnsafe(
    ...
    try {
        traceInfo('[PYTHON EXTENSION] Registering runtime manager with erdos');
        const runtimeManager = new LanguageManager(components.pythonEnvs);
        const registration = erdos.runtime.registerLanguageRuntimeManager('python', runtimeManager);
        context.subscriptions.push(registration);
        traceInfo('[PYTHON EXTENSION] Python runtime manager registered successfully');
        
        // Register Python completion provider for all Python documents (files and console)
        const completionProvider = new PythonConsoleCompletionProvider();
        ...
        traceError('[PYTHON EXTENSION] Failed to register Python runtime manager:', err);
    }
