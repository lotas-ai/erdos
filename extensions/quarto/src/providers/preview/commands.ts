...
// Import package checker service
let packageCheckerService: any = undefined;
export function setPackageCheckerService(service: any) {
  packageCheckerService = service;
}
...
abstract class PreviewDocumentCommandBase extends RenderCommand {
  ...
  protected async renderFormat(format?: string | null, onShow?: () => void) {
    ...
    if (targetEditor) {
      // Check packages before rendering (if package checker is available)
      if (packageCheckerService) {
        const content = targetEditor.document.getText();
        const languageId = targetEditor.document.languageId === 'quarto' ? 'r' : targetEditor.document.languageId;
        
        if (languageId === 'r' || languageId === 'python') {
          const shouldProceed = await packageCheckerService.checkPackagesBeforeExecution(
            content,
            languageId,
