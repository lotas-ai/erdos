...
export async function activate(context: vscode.ExtensionContext) {  
  ...
  // Register package checker service (will be set by workbench)
  const setPackageCheckerDisposable = vscode.commands.registerCommand(
    'quarto._setPackageCheckerService',
    (service: any) => {
      const { setPackageCheckerService } = require('./providers/preview/commands');
      setPackageCheckerService(service);
    }
  );
  context.subscriptions.push(setPackageCheckerDisposable);
