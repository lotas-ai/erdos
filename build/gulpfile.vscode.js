...
const vscodeResourceIncludes = [
	...
	'out-build/vs/workbench/contrib/welcomeGettingStarted/browser/resources/Welcome.md',
...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	const task = () => {
		...
		const quartoExtensionBuildRoot = path.join(root, '.build/extensions/quarto');
		const quartoDistMain = path.join(quartoExtensionBuildRoot, 'dist', 'src', 'main.js');
		const quartoOutDir = path.join(quartoExtensionBuildRoot, 'out');
		const quartoOutMain = path.join(quartoOutDir, 'src', 'main.js');

		if (fs.existsSync(quartoExtensionBuildRoot)) {
			if (!fs.existsSync(quartoDistMain)) {
				if (!fs.existsSync(quartoOutMain)) {
					throw new Error(`Quarto extension output missing at ${quartoOutMain}. Run "npm run compile-quarto" before packaging.`);
				}

				console.log('[quarto] Populating dist/ from out/ output for packaging');
				fs.rmSync(path.join(quartoExtensionBuildRoot, 'dist'), { recursive: true, force: true });
				copyDirectoryRecursiveSync(quartoOutDir, path.join(quartoExtensionBuildRoot, 'dist'));
			}
		}
