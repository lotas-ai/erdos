...
export class GettingStartedPage extends EditorPane {
	...
	private async runDispatchCommand(command: string, argument: string) {
		...
		switch (command) {
			...
			case 'launchErdosAssistantGuide': {
				try {
					await this.viewsService.openView(ERDOS_AI_VIEW_ID, true);
				const instructionsUri = FileAccess.asFileUri('vs/workbench/contrib/welcomeGettingStarted/browser/resources/erdosAiAssistant.md');
				await this.commandService.executeCommand('markdown.showPreviewToSide', instructionsUri);
				} catch (error) {
					onUnexpectedError(error);
					this.notificationService.error(localize('welcome.launchErdosAssistantGuideError', "We couldn't open the Erdos AI guide. {0}", getErrorMessage(error)));
				}
				break;
			}
			case 'allDone': {
	...
	private async buildCategoriesSlide() {
		...
		this.categoriesSlideDisposables.add(addDisposableListener(showOnStartupLabel, 'click', () => {
			...
			const header = $('.header', {},
				...
				$('p.subtitle.description', {}, localize({ key: 'welcome.erdos.subtitle', comment: ['Shown as subtitle on the Welcome page.'] }, "A secure, AI-native IDE built for data scientists."))
			);

			const leftColumn = $('.categories-column.categories-column-left', {},);
			const rightColumn = $('.categories-column.categories-column-right', {},);

		const assistantCtaTitle = localize('welcome.assistantWalkthrough.title', "Get started with Erdos AI Assistant");
		const assistantCtaDescription = localize('welcome.assistantWalkthrough.description', "Set up the Erdos AI assistant, connect your account, and learn the core workflows.");

		const assistantTitleContent = $('h3.category-title.max-lines-3');
		reset(assistantTitleContent, ...renderLabelWithIcons(assistantCtaTitle));

		const assistantDescriptionContent = $('.description-content');
		reset(assistantDescriptionContent, ...renderLabelWithIcons(assistantCtaDescription));

		const assistantIcon = $('span.icon-widget.codicon.codicon-star-full');
		const assistantCta = $('button.getting-started-category.featured.assistant-walkthrough', {
			'x-dispatch': 'launchErdosAssistantGuide',
			'title': assistantCtaDescription,
		},
			$('.featured-badge', {},
			...
			$('.main-content', {},
				assistantIcon,
				assistantTitleContent,
				$('.no-badge'),
				$('span.hide-category-placeholder')
			),
		...
		const layoutLists = () => {
			const listElement = gettingStartedList.getDomElement();
			if (gettingStartedList.itemCount) {
				this.container.classList.remove('noWalkthroughs');
				reset(rightColumn, listElement);
				const heading = listElement.querySelector('h2');
				if (heading) {
					heading.insertAdjacentElement('afterend', assistantCta);
				}
			}
			else {
