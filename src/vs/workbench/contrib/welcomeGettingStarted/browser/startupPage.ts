...
export class StartupPageRunnerContribution extends Disposable implements IWorkbenchContribution {
	...
	private async openGettingStarted(_showTelemetryNotice?: boolean) {
		if (this.openedWelcomePreview) {
			return;
		}
		...
		try {
			...
			await this.commandService.executeCommand('markdown.showPreview', welcomeUri, undefined, { locked: true });
		} catch (error) {
			// If preview fails (e.g., Markdown extension unavailable), fall back to opening the raw Markdown.
