...
export interface IErdosEnvironmentService {
	...
	// Package Checking
	checkMissingPackages(fileContent: string, filePath: string, languageId: 'python' | 'r', runtimeId?: string): Promise<string[]>;
	
	// Check packages before execution (for AI/console commands)
	checkAndPromptMissingPackages(code: string, languageId: 'python' | 'r', runtimeId?: string): Promise<boolean>;
}
