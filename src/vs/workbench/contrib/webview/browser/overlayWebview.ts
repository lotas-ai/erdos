...
export class OverlayWebview extends Disposable implements IOverlayWebview {
	...
	public get container(): HTMLElement {
		...
		if (!this._container) {
			...
			node.className = 'webview-overlay-container';
			node.style.position = 'absolute';
			node.style.overflow = 'hidden';
			node.style.userSelect = 'text';
			(node.style as any).webkitUserSelect = 'text';
			...
			const layoutContainer = this._layoutService.getContainer(this.window);
			layoutContainer.appendChild(node);
			
			// Also enable text selection on parent containers
			layoutContainer.style.userSelect = 'text';
			(layoutContainer.style as any).webkitUserSelect = 'text';
		}
