...
import { IReactComponentContainer } from '../../../../base/browser/ui/reactComponentContainer.js';
...
const AUTO_SCROLL_REENABLE_THRESHOLD = 48;
...
export const ErdosAi = React.forwardRef<ErdosAiRef, ErdosAiProps>((props, ref) => {	const [messages, setMessages] = useState<ConversationMessage[]>([]);
	...
	useEffect(() => {
		...
		const handleScrollPosition = () => {
			const distanceToBottom = container.scrollHeight - container.scrollTop - container.clientHeight;
			const isNearBottom = distanceToBottom <= AUTO_SCROLL_REENABLE_THRESHOLD;
			if (isNearBottom) {
				if (!autoScrollEnabledRef.current) {
		...
		container.addEventListener('scroll', handleScrollPosition, passiveOptions);
		container.addEventListener('wheel', handleUserIntent, passiveOptions);
		container.addEventListener('touchstart', handleUserIntent, passiveOptions);
		container.addEventListener('touchmove', handleUserIntent, passiveOptions);
		container.addEventListener('pointerdown', handleUserIntent, passiveOptions);
		window.addEventListener('keydown', handleKeyDown);

		handleScrollPosition();

		return () => {
			container.removeEventListener('scroll', handleScrollPosition, passiveOptions);
			container.removeEventListener('wheel', handleUserIntent, passiveOptions);
			container.removeEventListener('touchstart', handleUserIntent, passiveOptions);
			container.removeEventListener('touchmove', handleUserIntent, passiveOptions);
			container.removeEventListener('pointerdown', handleUserIntent, passiveOptions);
	...
	}, [messages, widgets, currentConversation?.streaming?.content, autoScrollEnabled, isLoadingConversation, widgetLayoutTrigger, messagesContainer]);
	...
	return (
		<div className="erdos-ai-container">
			...
			<div className="erdos-ai-messages" ref={messagesContainerRef} tabIndex={0}>
