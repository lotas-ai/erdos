...
export const FileChangesBar: React.FC<FileChangesBarProps> = ({
	...
	useEffect(() => {
		...
		const loadTrackedFiles = async () => {
			...
			try {
				...
				if (!isDisposed && loadId === latestLoadIdRef.current) {
	...
	const shouldHide = !hasLoadedOnce || !hasFiles;
	const containerClassName = `erdos-ai-file-changes-bar show-file-icons${shouldHide ? ' hidden' : ''}`;
