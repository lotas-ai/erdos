...
export function useMessageEditing({
	...
	const conversationIdRef = useRef<number | null>(currentConversation?.info.id ?? null);
	
	// Cancel edit when conversation changes
	useEffect(() => {
		const newConversationId = currentConversation?.info.id ?? null;
		
		if (editingMessageId !== null && newConversationId !== conversationIdRef.current) {
			setEditingMessageId(null);
			setEditingContent('');
		}
		
		conversationIdRef.current = newConversationId;
	}, [currentConversation?.info.id, editingMessageId]);
	...
	}, [editingMessageId, editingContent, erdosAiService, setCurrentConversation, setMessages, currentConversation]);
