...
import { getRuntimeIconBase64 } from '../runtimeIcons.js';

// Help event type
interface ShowHelpEvent {
	topic: string;
	[key: string]: any;
}
import { TopicWebviewDisplay } from '../models/helpEntry.js';
import { NotFoundDisplay } from '../models/notFoundDisplay.js';
...
export interface IErdosHelpService {
	...
	showNotFoundPage(searchQuery: string, suggestions: Array<{topic: string, languageId: string, languageName: string}>): Promise<void>;
...
class HelpService extends Disposable implements IErdosHelpService {
	...
	constructor(
		...
		@IProxyService proxyService: IProxyService,
	) {
		super();
		...
		this._proxyManager = new HelpProxyManager(proxyService, this._instantiationService);
		...
		_sessionManager.activeSessions.forEach(async (session) => {
			await this.connectRuntimeSession(session.sessionId);
		});

		this._register(
			_sessionManager.onDidStartSession(async (session) => {
				await this.connectRuntimeSession(session.sessionId);
			})
		);
	}
	...
	async showNotFoundPage(
		searchQuery: string, 
		suggestions: Array<{topic: string, languageId: string, languageName: string}>
	): Promise<void> {
		await this._viewsService.openView(ERDOS_HELP_VIEW_ID, false);

		const primaryLanguageId = suggestions.length > 0 ? suggestions[0].languageId : 'r';
		const primaryLanguageName = suggestions.length > 0 ? suggestions[0].languageName : 'R';
	...
	getActiveHelpRuntimes(): IHelpRuntime[] {
		...
		const sessions = this._sessionManager.activeSessions;
		...
		for (const runtime of registeredRuntimes) {
			...
			if (!existing || (isActive && !existing.isActive)) {
				runtimesByLanguageId.set(runtime.languageId, {
					...
					isActive: isActive,
					base64EncodedIconSvg: getRuntimeIconBase64(runtime.languageId)
				});
			}
		}
...
registerSingleton(IErdosHelpService, HelpService, InstantiationType.Eager);
