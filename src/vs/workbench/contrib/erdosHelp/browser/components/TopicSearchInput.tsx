...
export const TopicSearchInput: React.FC<TopicSearchInputProps> = ({
	...
	const handleNoResultsEnter = useCallback(async (searchQuery: string) => {
		setShowDropdown(false);
		
		const suggestions = await services.topicQueryService.queryAllLanguages(searchQuery);
		const limitedSuggestions = suggestions.slice(0, 10);
		
		await services.erdosHelpService.showNotFoundPage(searchQuery, limitedSuggestions);
	}, [services.topicQueryService, services.erdosHelpService]);
		...
		switch (e.key) {
			...
			case 'Enter':
				...
				if (showDropdown && sortedResults.length > 0) {
					...
					if (indexToSelect < sortedResults.length) {
						const selectedResult = sortedResults[indexToSelect];
						handleTopicSelection(selectedResult.topic, selectedResult.languageId);
					}
				} else if (query.trim()) {
					handleNoResultsEnter(query.trim());
				}
				break;
			case 'Escape':
				e.preventDefault();
	...
	}, [showDropdown, sortedResults, selectedIndex, query, results, handleNoResultsEnter, handleTopicSelection]);
	...
	useEffect(() => {
		...
		if (showDropdown) {
			...
			if (dropdownRef.current && inputRef.current) {
				...
				if (dropdown) {
					const dropdownWidth = Math.max(inputRect.width, 280);
					...
					if (rightEdge > viewportWidth) {
						// Reposition to keep within viewport with 8px padding
						leftPosition = Math.max(8, viewportWidth - dropdownWidth - 8);
					}
