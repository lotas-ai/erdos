...
const enum PlotCommandIdentifiers {
	OpenView = 'workbench.action.erdosPlots.openView',
...
export function initializeCommandRegistry() {
	...
	registerAction2(class extends Action2 {
		constructor() {
			super({
				id: PlotCommandIdentifiers.OpenView,
				title: {
					value: localize('command.openPlotsView', "Open Plots"),
					original: 'Open Plots'
				},
				f1: true,
				category
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			await accessor.get(IViewsService).openView(ERDOS_PLOTS_VIEW_ID, true);
		}
	});
