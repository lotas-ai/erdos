...
import { getRuntimeIconBase64 } from '../../../../erdosHelp/browser/runtimeIcons.js';
	...
	async getChildren(element: FileGroup | { element: PlotHistoryTreeElement } | null): Promise<{ element: PlotHistoryTreeElement }[]> {
		if (!element || element === null) {
			...
			allGroups.forEach(group => {
				// For console plots, group by language internally but display as "Console"
				// For file plots, group by file path
				let key: string;
				if (group.source === 'Console') {
					// Console plots - group by language
					if (group.language) {
						key = `__console_${group.language}__`;
					} else {
						key = '__console_unknown__';
					}
				} else {
					// File or notebook plots - use the source as-is
					key = group.source;
				}
...
export class PlotHistoryViewPane extends ViewPane {
	...
	private _createTree(): void {
		...
		const fileGroupRenderer = new FileGroupRenderer(this._erdosPlotsService);
