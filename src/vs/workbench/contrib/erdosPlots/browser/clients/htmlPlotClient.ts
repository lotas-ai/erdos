...
import { ShowHtmlFileEvent } from '../../../../services/languageRuntime/common/languageRuntimeMessageTypes.js';
...
export class HtmlPlotClient extends WebviewPlotClient {
	...
	get uri(): URI {
		const filePath = this._event.path || this._event.file_path;
	...
	protected override async initializeView() {
		...
		const filePath = this._event.path || this._event.file_path;
		const pathUri = URI.parse(filePath);
		const isHttpUrl = pathUri.scheme === 'http' || pathUri.scheme === 'https';

		const proxyUrl = isHttpUrl 
			? await this._proxyService.createHttpProxy(filePath, filePath)
			: await this._proxyService.createHtmlProxy(filePath, filePath);

		if (!proxyUrl) {
			throw new Error(`Failed to start proxy server for ${filePath}`);
		}
