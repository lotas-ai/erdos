...
async function webviewPreloads(ctx: PreloadContext) {
	...
	window.addEventListener('message', async rawEvent => {
		...
		switch (event.data.type) {
			case 'erdosRender': {
				if (event.data.type === 'erdosRender') {
					...
					outputRunner.enqueue(data.outputId, async signal => {
						...
						if (!element) {
							...
							element.classList.add('erdos-output-container');
					...
					const item = createOutputItem(
						...
						data.mimeType,
						data.metadata,
						new Uint8Array(data.valueBytes),
						[],
					);

						await renderers.render(item, data.rendererId, element, signal);

						postNotebookMessage('erdosRenderComplete', { outputId: data.outputId });
					});
				}
				break;
			}
