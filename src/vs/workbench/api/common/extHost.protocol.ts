...
// #region Runtime (Language Runtimes)
...
export interface MainThreadRuntimeShape extends IDisposable {
	$registerLanguageRuntime(metadata: any): void;
	$unregisterLanguageRuntime(runtimeId: string): void;
	$completeLanguageRuntimeDiscovery(): void;
	$getPreferredRuntime(languageId: string): Promise<any>;
	$getActiveSessions(): Promise<any[]>;
	$getForegroundSession(): Promise<string | undefined>;
	$getNotebookSession(notebookUri: UriComponents): Promise<string | undefined>;
	$selectLanguageRuntime(runtimeId: string): Promise<void>;
	$getRegisteredRuntimes(): Promise<any[]>;
	$startLanguageRuntime(runtimeId: string, sessionName: string, sessionMode: number, notebookUri: UriComponents | undefined): Promise<string>;
	$restartSession(handle: number): Promise<void>;
	$interruptSession(handle: number): Promise<void>;
	$focusSession(handle: number): void;
	$executeCode(languageId: string, extensionId: string, code: string, focus: boolean, allowIncomplete?: boolean, mode?: any, errorBehavior?: any, executionId?: string, batchId?: string, filePath?: string): Promise<string>;
	$emitLanguageRuntimeMessage(handle: number, handled: boolean, message: SerializableObjectWithBuffers<any>): void;
	$emitLanguageRuntimeState(handle: number, clock: number, state: any): void;
...
export interface ExtHostRuntimeShape {
	$createLanguageRuntimeSession(runtimeMetadata: any, sessionMetadata: any): Promise<RuntimeInitialState>;
	$validateLanguageRuntimeMetadata(metadata: any): Promise<any>;
	$validateLanguageRuntimeSession(metadata: any, sessionId: string): Promise<boolean>;
	$startLanguageRuntime(handle: number): Promise<any>;
	$executeCode(handle: number, code: string, id: string, mode: any, errorBehavior: any, executionId?: string, batchId?: string, filePath?: string): Promise<void>;
	$isCodeFragmentComplete(handle: number, code: string): Promise<any>;
	$createClient(handle: number, id: string, type: any, params: any, metadata?: any): Promise<void>;
	$listClients(handle: number, type?: any): Promise<Record<string, any>>;
	$sendClientMessage(handle: number, clientId: string, data: any): Promise<void>;
	$removeClient(handle: number, clientId: string): Promise<void>;
	$setWorkingDirectory(handle: number, directory: string): Promise<void>;
	$interruptLanguageRuntime(handle: number): Promise<void>;
	$replyToInput(handle: number, parentId: string, value: string): Promise<void>;
	$restartSession(handle: number, workingDirectory?: string): Promise<void>;
	$shutdownLanguageRuntime(handle: number, exitReason: any): Promise<void>;
	$forceQuitLanguageRuntime(handle: number): Promise<void>;
	$discoverLanguageRuntimes(disabledLanguageIds: string[]): void;
	$recommendWorkspaceRuntimes(disabledLanguageIds: string[]): Promise<any[]>;
	$notifyForegroundSessionChanged(sessionId: string | undefined): void;
	$notifyCodeExecuted(event: any): void;
}
...
export interface ProxyOptions {
	...
	styles?: string[];
	scripts?: string[];
	helpStyles?: Record<string, string | number>;
}

export interface IMainThreadProxy {
	$createProxy(targetOrigin: string, options: ProxyOptions): Promise<string>;
	$disposeProxy(proxyId: string): Promise<void>;
}

export interface MainThreadProxyShape extends IDisposable, IMainThreadProxy {
}

export interface ExtHostProxyShape {
	$createProxy(targetOrigin: string, options: ProxyOptions): Promise<string>;
	$disposeProxy(proxyId: string): Promise<void>;
}
...
export const MainContext = {
	MainThreadRuntime: createProxyIdentifier<MainThreadRuntimeShape>('MainThreadRuntime'),
	...
	MainThreadProxy: createProxyIdentifier<MainThreadProxyShape>('MainThreadProxy'),
};

export const ExtHostContext = {
	ExtHostRuntime: createProxyIdentifier<ExtHostRuntimeShape>('ExtHostRuntime'),
	...
	ExtHostProxy: createProxyIdentifier<ExtHostProxyShape>('ExtHostProxy'),
};
