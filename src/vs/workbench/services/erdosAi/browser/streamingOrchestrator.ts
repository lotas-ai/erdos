...
export class StreamingOrchestrator extends Disposable implements IStreamingOrchestrator {
	...
	/**
	 * Mark the active batch as stream-complete when the backend finishes sending data.
	 * This is a safety net for providers that do not emit an explicit `done` event
	 * after tool calls (e.g. view_image), which previously left the state machine
	 * waiting indefinitely for completion signals.
	 */
	markCurrentBatchStreamComplete(): void {
