...
export class ErdosAiServiceCore extends Disposable implements IErdosAiServiceCore {
	...
	async executeStreamingForOrchestrator(message: string, userMessageId: number, requestId: string): Promise<void> {
		try {
			...
			this.logService.error(`[STATE MACHINE] executeStreamingForOrchestrator failed request=${requestId}`, error);
			this.hideThinkingMessage();
	...
	private async determineNextAction(needsInitialApiCall: boolean): Promise<'api_call' | 'process_widget_decision' | 'wait_for_processing' | 'wait_for_widget_decision' | 'complete' | 'error'> {
		...
		if (batchStatus === 'pending') {
			...
			this.logService.warn(`[STATE MACHINE] determineNextAction batchError request=${this.currentRequestId} batch=${batchId}`);
			return 'error';
	...
	private async processWidgetDecision(): Promise<void> {
		...
		if (result) {
			...
			const branches = currentBatchId ? this.branchManager.getBatchBranches(currentBatchId) : [];
			const branch = branches.find(b => b.messageId === decision.messageId);
			if (!branch) {
				this.logService.warn(`[WIDGET DECISION] Branch not found for messageId=${decision.messageId} batch=${currentBatchId}`);
			}
	...
	private async makeApiCall(): Promise<void> {
		...
		const requestMessages = this.prepareMessagesWithPlotImages(messages);
		const requestId = this.currentRequestId!;

		const contextData = await this.contextService.prepareContextForBackend(requestMessages);
		...
		return new Promise<void>((resolve, reject) => {
			...
			let receivedContentEvents = false;
			this.backendClient.sendStreamingQuery(
				...
				async () => {
					...
					// Ensure batches are marked complete even if no explicit 'done' event was received
					this.streamingOrchestrator.markCurrentBatchStreamComplete();
					
					this.hideThinkingMessage();
					if (!receivedEvents) {
						this.logService.warn(`[STATE MACHINE] No streaming events received for request=${requestId}`);
					} else if (!receivedContentEvents) {
						this.logService.warn('[STATE MACHINE] Streaming completed without content events; skipping fallback assistant message');
					}
	...
	private prepareMessagesWithPlotImages(messages: ConversationMessage[]): ConversationMessage[] {
		if (!messages || messages.length === 0) {
			return messages;
		}

		const clonedMessages = messages.map(message => this.cloneMessageForRequest(message));
		const latestUserMessage = this.findLatestUserQueryMessage(clonedMessages);
		if (!latestUserMessage) {
			return clonedMessages.filter((msg) => !this.isProceduralImageMessage(msg));
		}
		...
		for (let i = latestUserMessage.index + 1; i < clonedMessages.length; i++) {
			...
			const clonedContent = message.content.map(item => {
			...
			const imageItems = clonedContent.filter(item => !!item && typeof item === 'object' && (item as any).type === 'input_image' && typeof (item as any).image_url === 'string');
			if (imageItems.length === 0) {
				continue;
			}

			const imageKey = imageItems.map(item => (item as any).image_url as string).join('|');
			const isProceduralImage = this.isProceduralImageMessage(message);
			if (seenImageKeys.has(imageKey)) {
				if (isProceduralImage) {
			...
			imageSegments.push(...imageItems);

			if (isProceduralImage) {
				removalIndices.add(i);
			}

			if (imageSegments.length >= 3) {
				if (imageSegments.length > 3) {
					imageSegments.length = 3;
				}
				break;
			}
		}

		const userMessage = clonedMessages[latestUserMessage.index];
		let userContent: any[];
		if (Array.isArray(userMessage.content)) {
			userContent = userMessage.content.map(item => {
				if (typeof item === 'object' && item !== null) {
					return { ...item };
				}
				return item;
			});
		} else if (typeof userMessage.content === 'string' && userMessage.content.length > 0) {
			userContent = [{ type: 'input_text', text: userMessage.content }];
		} else {
			userContent = [];
		}

		if (imageSegments.length > 0) {
			userContent.push(...imageSegments);
			userMessage.content = userContent as any;
		}

		for (let i = 0; i < clonedMessages.length; i++) {
			if (i === latestUserMessage.index) {
				continue;
			}
			if (this.isProceduralImageMessage(clonedMessages[i])) {
				removalIndices.add(i);
			}
		}
		...
		return clonedMessages.filter((_, index) => !removalIndices.has(index));
	}

	private isProceduralImageMessage(message: ConversationMessage | undefined): boolean {
		if (!message || !message.procedural) {
			return false;
		}
		...
		return message.content.some(item => !!item && typeof item === 'object' && (item as any).type === 'input_image' && typeof (item as any).image_url === 'string');
	}

	private cloneMessageForRequest(message: ConversationMessage): ConversationMessage {
		...
		if (message.function_call) {
			cloned.function_call = { ...message.function_call };
		}
		return cloned;
	}

	private findLatestUserQueryMessage(messages: ConversationMessage[]): { index: number; message: ConversationMessage } | null {
		for (let i = messages.length - 1; i >= 0; i--) {
			const message = messages[i];
			if (message.role === 'user' && !message.function_call) {
