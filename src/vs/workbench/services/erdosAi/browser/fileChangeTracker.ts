...
export class FileChangeTracker extends Disposable implements IFileChangeTracker {
	...
	private readonly notebookOverviewSummaries = new Map<string, Map<number, NotebookOverviewEntry>>();
	...
	public getNotebookOverviewSummary(uri: URI): Map<number, NotebookOverviewEntry> | undefined {
		const summary = this.notebookOverviewSummaries.get(uri.toString());
		if (!summary) {
			return undefined;
		}
		return new Map(Array.from(summary.entries(), ([cellIndex, entry]) => [cellIndex, {
			added: [...entry.added],
	...
	private clearAutoAcceptZones(uriString: string): void {
		...
		this.notebookOverviewSummaries.delete(uriString);
	...
	if (isNotebook) {
		...
		try {
			...
			// For regular files, prefer the live model so unsaved accepts are reflected immediately
			const model = this.modelService.getModel(uri);
			if (model) {
				fileTracking.accepted_content = model.getValue();
			} else {
		...
		// Notify listeners that all pending diff sections were accepted
		this._onDiffSectionChanged.fire({ uri, action: 'accept', sectionId: 'all' });
	}
	...
	async rejectAllAutoAcceptChanges(uri: URI): Promise<void> {
		...
		// Notify listeners that all pending diff sections were rejected
		this._onDiffSectionChanged.fire({ uri, action: 'reject', sectionId: 'all' });
	}
	...
	const overviewSummary = new Map<number, NotebookOverviewEntry>();
	for (const [cellIndex, cellLineDiffs] of cellDiffMap) {
		const added: number[] = [];
		const deleted: number[] = [];

		for (const diff of cellLineDiffs) {
			if (diff.type === 'added') {
				added.push(diff.lineNumber);
			} else if (diff.type === 'deleted') {
				deleted.push(diff.lineNumber);
			}
		}

		if (added.length || deleted.length) {
			const uniqueAdded = Array.from(new Set(added)).sort((a, b) => a - b);
			const uniqueDeleted = Array.from(new Set(deleted)).sort((a, b) => a - b);
			overviewSummary.set(cellIndex, { added: uniqueAdded, deleted: uniqueDeleted });
		}
	}
	...
	if (overviewSummary.size > 0) {
		this.notebookOverviewSummaries.set(uri.toString(), overviewSummary);
	} else {
		this.notebookOverviewSummaries.delete(uri.toString());
	}
	...
	// Parse accepted content as JSON
		...
		// Track which cells had content BEFORE we remove deleted lines (to know which became empty)
		const cellsWithContentBefore = new Set<number>();
		acceptedNotebook.cells.forEach((cell: any, index: number) => {
			if (cell.source && Array.isArray(cell.source) && cell.source.length > 0) {
				cellsWithContentBefore.add(index);
			}
		});
		...
		acceptedNotebook.cells = acceptedNotebook.cells.filter((cell: any, index: number) => {
			...
			const hadContentBefore = cellsWithContentBefore.has(index);
			const isEmpty = !cell.source || (Array.isArray(cell.source) && cell.source.length === 0);
			
			if (isEmpty) {
				if (isNewlyCreated || !hadContentBefore) {
					// Keep: newly created empty cells OR cells that were already empty in baseline
					return true;
				} else {
					// Delete: cells that had content before but are now empty (content was deleted)
	...
	private async notebookRejectDiffSection(uri: URI, diffSectionId: string): Promise<void> {		
		...
		if (storedInfo.type === 'added-only') {
			...
			if (currentCell) {
				...
				// Find the accepted cell (if it exists)
				const acceptedCell = acceptedNotebook.cells.find((cell: any) => {
					const cellId = cell.metadata?.erdosAi_cellId;
					return cellId === currentCellId;
				});
				
				// Check if the cell doesn't exist OR exists but is empty in the accepted baseline
				const acceptedCellIsEmpty = acceptedCell && (!acceptedCell.source || 
					(Array.isArray(acceptedCell.source) && acceptedCell.source.length === 0));

				if (currentCellId && (!acceptedCell || acceptedCellIsEmpty)) {
					// Delete the entire cell from the current notebook
					notebookModel.applyEdits([{
						editType: 1, // CellEditType.Replace
						index: cellIndex,
						count: 1,
					...
					// CRITICAL: Also remove the cell from the accepted baseline
					// Otherwise applyAutoAcceptHighlighting will recreate it as an empty cell
					if (acceptedCell) {
						const acceptedCellIndex = acceptedNotebook.cells.findIndex((cell: any) => 
							cell.metadata?.erdosAi_cellId === currentCellId
						);
						if (acceptedCellIndex !== -1) {
							acceptedNotebook.cells.splice(acceptedCellIndex, 1);
							notebookFileTracking.accepted_content = JSON.stringify(acceptedNotebook, null, 2);
							await this.fileService.writeFile(notebookFileTrackingPath, VSBuffer.fromString(JSON.stringify(notebookFileTracking, null, 2)));
						}
					}
