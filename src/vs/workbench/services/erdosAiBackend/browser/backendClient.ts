...
export class BackendClient extends Disposable implements IBackendClient {
	...
	private async buildRequestData(
		...
		if (!isBYOK) {
			...
			if (!auth) {
				throw new Error('No API key configured. Please visit Erdos AI settings (click on the gear icon in the top right of this pane). You may click the sign in button to link your Lotas account, or set your own OpenAI/Anthropic API key at the bottom of the Erdos AI settings page.');
			}
		}
