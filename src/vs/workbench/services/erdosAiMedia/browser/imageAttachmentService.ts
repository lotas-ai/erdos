...
import { IErdosPlotsService, IErdosPlotClient } from '../../../contrib/erdosPlots/common/erdosPlotsService.js';
...
export class ImageAttachmentService extends Disposable implements IImageAttachmentService {
	...
	constructor(
		...
		@IErdosPlotsService private readonly _plotsService: IErdosPlotsService,
		@IImageProcessingManager private readonly _imageProcessingManager: IImageProcessingManager
	) {
		super();
	...
	async attachPlotFromService(plotId: string): Promise<IAttachedImage | null> {
		...
		const plotClient = this._plotsService.allPlots.find(plot => plot.id === plotId);
		if (!plotClient) {
			throw new Error(`Plot '${plotId}' is no longer available.`);
		}

		const extraction = await this._imageProcessingManager.extractImageDataFromPlotClient(plotClient);
		if (!extraction.success) {
			throw new Error(extraction.warning || 'Failed to extract plot image data.');
		}

		const normalized = this._normalizeExtractedPlot(extraction);
		const originalHash = await this.hashBuffer(normalized.buffer);
		const isDuplicate = this._attachedImages.some(img => img.originalHash === originalHash);
		if (isDuplicate) {
			throw new Error('This plot is already attached to the conversation.');
		}

		const imagesDir = await this._getImagesDirectory();
		await this._ensureDirectoryExists(imagesDir);

		const defaultName = this._suggestPlotFilename(plotClient, normalized.mimeType);
		const localFileName = await this._generateUniqueFileName(imagesDir, defaultName);
		const localUri = URI.joinPath(imagesDir, localFileName);
		await this._fileService.writeFile(localUri, normalized.buffer);

		const attachedImage: IAttachedImage = {
			id: this.generateId(),
			filename: localFileName,
			originalPath: `plot:${plotClient.id}`,
	...
	async getAvailablePlots(): Promise<Array<{ id: string; metadata: any }>> {
		const plots = [...this._plotsService.allPlots];
		plots.sort((a, b) => {
			const aTime = new Date(a.metadata.created).getTime();
			const bTime = new Date(b.metadata.created).getTime();
			return bTime - aTime;
		});
		return plots.map((plot: IErdosPlotClient) => ({
			id: plot.id,
			metadata: {
				...plot.metadata,
				label: this._buildPlotLabel(plot)
			}
		}));
	}
	...
	private _normalizeExtractedPlot(extraction: {
		success: boolean;
		base64_data: string;
		...
		format: string;
		warning?: string;
	}): { buffer: VSBuffer; mimeType: string; base64: string; format: string } {
		const sanitizedBase64 = extraction.base64_data.replace(/\s/g, '');
		const buffer = decodeBase64(sanitizedBase64);
		const mimeType = this._mimeFromFormat(extraction.format) ?? 'image/png';
		const normalizedBase64 = encodeBase64(buffer);
		return {
			buffer,
			mimeType,
	...
	private _mimeFromFormat(format: string | undefined): string | undefined {
		if (!format) {
			return undefined;
		}
	...
	private _suggestPlotFilename(plot: IErdosPlotClient, mimeType: string): string {
		const extension = this._extensionFromMime(mimeType);
		const created = plot.metadata.created ? new Date(plot.metadata.created).toISOString().replace(/[:]/g, '-') : undefined;
		const language = plot.metadata.language ? plot.metadata.language.toLowerCase() : undefined;
		const parts = [language || 'plot', created || plot.id];
		return `${parts.filter(Boolean).join('_')}.${extension}`;
	}

	private _buildPlotLabel(plot: IErdosPlotClient): string {
		const created = plot.metadata.created ? new Date(plot.metadata.created).toLocaleTimeString() : undefined;
		const language = plot.metadata.language ? plot.metadata.language.toUpperCase() : undefined;
		const pieces = [language ? `${language} plot` : 'Plot', created].filter(Boolean);
		return pieces.join(' • ');
	}
}
