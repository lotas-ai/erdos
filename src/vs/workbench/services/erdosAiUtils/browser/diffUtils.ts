...
class DiffStorage {
	...
	async storeNotebookDiff(
		...
		try {
			...
			// Get cell types from the parsed notebook structure
			const notebook = parseResult.notebook;
			
			for (const [cellIndex, lineDiffs] of cellDiffs.entries()) {
				// Get cell type from parsed notebook cells
				const cellType = notebook.cells[cellIndex]?.cell_type || 'code';
			...
			console.error(`Failed to compute notebook diff for ${filePath}:`, error);
