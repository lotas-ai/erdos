...
export class DocumentManager extends Disposable implements IDocumentManager {
	...
	async getAllOpenDocuments(includeContent: boolean = true): Promise<DocumentInfo[]> {
		...
		for (const editor of allEditors) {
			...
			// Skip deleted files - only include files that exist or are untitled
			if (resource.scheme === 'file') {
				const fileExists = await this.fileService.exists(resource);
				if (!fileExists) {
					// File was deleted but is still open in the editor - skip it
					continue;
				}
			}
