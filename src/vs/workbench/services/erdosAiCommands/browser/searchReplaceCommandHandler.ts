	...
	async validateAndProcessSearchReplace(functionCall: any, messageId: number, relatedToId: number, requestId: string): Promise<{success: boolean, errorMessage?: string}> {
		try {
			...
			if (oldString === '') {
				...
				// For notebook files, use notebook-specific diff algorithm
				if (isNotebook) {
					// For notebook file creation with empty old_string, treat currentContent as old and newContent as new
					await diffStore.storeNotebookDiff(
						'',  // oldString is empty for creation
						newString,  // newString is what we're adding
						messageId.toString(),
						filePath,
