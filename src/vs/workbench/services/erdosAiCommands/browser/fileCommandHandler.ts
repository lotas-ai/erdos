...
export class FileCommandHandler extends Disposable implements IFileCommandHandler {
	...
	private lastResolvedFilePath: string | undefined;
		...
		try {
			...
			if (filename && this.commonUtils.getFileExtension(filename).toLowerCase() === 'ipynb') {
				...
				successMessage = 'Notebook cells executed successfully - returning control to orchestrator';
					...
					const attributionFilePath = this.lastResolvedFilePath ?? filename;
					this.lastResolvedFilePath = undefined;
					...
					executionResult = await this.consoleCommandHandler.executeConsoleCommandWithOutputCapture(
						executableCommand,
						callId,
						language,
						{
	...
	async processFileForExecution(functionCall: any, callId: string): Promise<string> {
		try {
			...
			const resolvedPath = fileUri?.fsPath ?? fileResult.relativePath ?? filename;
			...
			const preparedContent = await this.extractFileContentForWidgetExecution(filename, startLine, endLine);

			if (preparedContent.startsWith('Error:')) {
				return preparedContent;
			}

			this.lastResolvedFilePath = resolvedPath;
			return preparedContent;
