...
import { RuntimeCodeExecutionMode, RuntimeErrorBehavior, RuntimeState } from '../../../services/languageRuntime/common/languageRuntimeTypes.js';

// Message types for runtime output handling
...
import { CodeAttributionSource, IConsoleCodeAttribution, ILanguageRuntimeCodeExecutedEvent } from '../../../services/languageRuntime/common/codeExecution.js';
...
constructor(
		...
		@IPackageCheckerService private readonly packageCheckerService: IPackageCheckerService,
	) {
super();
	}
	...
	async focusConsoleForLanguage(language: string): Promise<void> {
		...
		this.sessionManager.foregroundSession = session;
	}

	private async getOrCreateSession(language: string) {
		let session = this.sessionManager.getConsoleSessionForLanguage(language);
		if (!session) {
			// Show the quickpick to let user select a runtime
			await this.commandService.executeCommand('erdos.languageRuntime.startNewSession');
			
			// Check again if a session was started
			session = this.sessionManager.getConsoleSessionForLanguage(language);
			if (!session) {
				throw new Error(`No ${language} session was started. Please start a session to execute code.`);
			}
		}
		return session;
	}

	async executeConsoleCommandWithOutputCapture(
		command: string,
		executionId: string,
		language: string = 'r',
		options?: { batchId?: string; filePath?: string }
	): Promise<string> {
		// Check for missing packages before execution (for R and Python)
		// This BLOCKS until user decides what to do
		if (language === 'python' || language === 'r') {
			try {
				const shouldProceed = await this.packageCheckerService.checkPackagesBeforeExecution(command, language as 'python' | 'r', true);
				if (!shouldProceed) {
					return 'Error: Execution cancelled by user due to missing packages.';
				}
			} catch (error) {
		...
		let finalSession = this.sessionManager.getConsoleSessionForLanguage(language);
		
		if (!finalSession) {
			// Show the quickpick to select and start a session
			await this.commandService.executeCommand('erdos.languageRuntime.startNewSession');
			
			// Check again if a session was started
			finalSession = this.sessionManager.getConsoleSessionForLanguage(language);
		...
		if (batchId) {
			attribution.batchId = batchId;
		}
		if (filePath) {
			attribution.metadata = { filePath };
		}
		...
		const event: ILanguageRuntimeCodeExecutedEvent = {
			...
			attribution,
			code: command,
			languageId: finalSession.runtimeMetadata.languageId,
			runtimeName: finalSession.runtimeMetadata.runtimeName,
			errorBehavior: RuntimeErrorBehavior.Continue,
			mode: RuntimeCodeExecutionMode.Interactive
		};

		// Notify console listeners so the executed code is displayed before output streaming begins.
		this.consoleService.recordCodeExecution(event);
		...
		return new Promise<string>((resolve, reject) => {
			try {
				...
				disposables.push(finalSession.onDidReceiveRuntimeMessageOutput((message: ILanguageRuntimeMessageOutput) => {
					...
					// Use state changes to detect when execution completes
				let wasExecuting = false;
				disposables.push(finalSession.onDidChangeRuntimeState((state) => {
					if (state === RuntimeState.Busy) {
						wasExecuting = true;
					} else if (wasExecuting && state === RuntimeState.Idle) {
						cleanup();
