...
// --- Start Erdos ---
// Tracks whether cursor is in a context where tab should trigger suggestions (similar to WordContextKey)
class TabSuggestContextKey {
	...
	constructor(
		...
		this._ckInTabSuggestContext = SuggestContext.InTabSuggestContext.bindTo(contextKeyService);
	...
	private _update(): void {
		...
		if (this._enabled) {
			const checkContext = () => {
				...
				const lineContent = this._editor.getModel().getLineContent(position.lineNumber);
				const leadingText = lineContent.substring(0, position.column - 1);

				// Set to true only if there's non-whitespace content before cursor
				this._ckInTabSuggestContext.set(leadingText.trim().length > 0);
			};
			this._selectionListener = this._editor.onDidChangeCursorSelection(checkContext);
			checkContext();
...
registerEditorCommand(new SuggestCommand({
	...
	precondition: ContextKeyExpr.and(
		...
		SuggestContext.InTabSuggestContext,
